...
from cued_sf2_lab.dct import dct_ii, dct_iv, regroup, colxfm, dct, idct, dctbpp
    ...
    def test_basic(self):
        ...
        npt.assert_allclose(
            ...
            [
                ...
                [0.5, -0.5, -0.5, 0.5],
                [0.27059, -0.65328, 0.65328, -0.27059],
            ],
            atol=1e-5,
        )
...
class TestDCT:
    def test_dct(self):
        ...
        Y = dct(X, N)
        Z = idct(Y, N)
        npt.assert_allclose(X, Z, atol=1e-7)

    def test_dctbpp(self):
        X = np.arange(4 * 4).reshape(4, 4)
        N = 2
        Y = dct(X, N)
        Yr = regroup(Y, N)
        npt.assert_equal(dctbpp(Yr, N), 8.0)
        npt.assert_equal(dctbpp(Yr, 4), 0.0)
